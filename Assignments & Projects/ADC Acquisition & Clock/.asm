******************************************************************************************
*                                                                                        *
* Title:          Analog Signal Acquisition and Digital Clock with Wave Generation       *
*                                                                                        *
...
*                                                                                        *
* Functionality:  a) Analog signal acquisition through ADC at 8 kHz sampling rate        *
*                 b) Generates various waveforms while displaying a digital clock:       *
*                 c) Waveform generation (sawtooth, triangle, square) at multiple rates  *
*                 d) 24-hour digital clock runs continuously in background               *
*                 e) 7-segment display shows selected time component                     *
*                 f) Commands processed through SCI serial communication                 *
*                                                                                        *
* Algorithm:      Uses OC5 timer interrupts at 125us for waveform generation and ADC     *
*                 Uses RTI interrupts at 2.5ms intervals for timekeeping                 *
*                 Command parsing determines ADC, wave type and clock functions          *
*                                                                                        *
* Register Usage: A, B: Character processing, wave calculations and ADC processing       *
...
ATDDR0H     EQU         $0090     ; ATD Result Register 0 High
ATDDR0L     EQU         $0091     ; ATD Result Register 0 Low
ATDDR7H     EQU         $009E     ; ATD Result Register 7 High
ATDDR7L     EQU         $009F     ; ATD Result Register 7 Low
...
waveType    DS.B   1              ; 0=none of HW10, 1=sawtooth, 2=sawtooth 125Hz, 
...
waveDir     DS.B   1              ; Direction for wave (0=up, 1=down)
waveCount   DS.B   1              ; Counter for square wave timing
pointCount  DS.W   1              ; Number of points generated out of POINTS_MAX

; ADC variables - Controls analog signal acquisition
adcActive   DS.B   1              ; Status flag: 0=inactive, 1=active
adcValue    DS.B   1              ; Current ADC value (0-255)
adcCount    DS.W   1              ; Number of ADC samples acquired
...
displayMode DS.B   1              ; Display mode: 1=hour, 2=min, 3=sec
...
; Interrupt vector section
            ...
            ORG    $FFE4          ; Timer channel 5 interrupt vecto[r
            DC.W   oc5isr
...
invalidTimeFormat   EQU   *  ; Forwarding for long branch targets
...
Entry
            ...
            STAA   ATDCTL2
            LDAA   #%00001000     ; Single conversion per sequence, no FIFO as was discussed in today's class
            STAA   ATDCTL3
            ...
            ; Initialize ADC acquisition variables
            CLR    adcActive      
            CLR    adcValue       
            CLR    adcCount       
            CLR    adcCount+1     
            ...
            ; Initialize Timer for wave generation and ADC
            BSET   TIOS,%00100000   ; Set channel 5 for Output Compare as was requested by professor on Monday April 14
...
mainLoop    
            LDAA   adcActive        ; Check if ADC acquisition is active
            BNE    skipMainProcessing ; Skip normal processing during ADC acquisition
            
            JSR    updateTime       ; update time
            JSR    checkInput       ; Process any keyboard input from user (if any)
            
            LDAA   cmdEntered       ; Check if a command has been entered
            LBEQ    checkActiveStatus ; If no command, check active processes
            
            JSR    processCommand   ; Process the entered command
            CLR    cmdEntered       ; Clear the command entered flag
            LBRA    checkActiveStatus
            
skipMainProcessing
            ; Only check ADC status when in ADC mode, skip other processing
            LDD    adcCount
            CPD    #POINTS_MAX+1   ; Check for 2049 samples to account for the skipped first sample
            ; As mentioned before, the reason we do 2049, is because the first value I had was stale data.
            ; To avoid changing the core code from HW10, I just used a simple shift of 1 to the right
            ; Instead of -1 to 2046 I can now process 0-2047! It still gives the full 2048 samples :)
            LBLO    mainLoop       ; Continue ADC acquisition if not done
            
            ; ADC complete - clean up and notify user
            CLR    adcActive        ; Reset active flag
            BCLR   TIE,%00100000    ; Disable timer interrupt
            
            JSR    nextline
            LDX    #processIndent   ; indent tracking
            JSR    printmsg
            LDX    #adcCompleteMsg
            JSR    printmsg
            ...
            ; Reset command processing variables for next command 
            ;(was made because some stale values were loading when using HW10)
...
checkActiveStatus
            ; Check if ADC acquisition is active (double check)
            LDAA   adcActive        
            BNE    skipMainProcessing ; If active, skip to ADC processing
            
            ; Check if wave generation is active
            LDAA   waveActive       
            LBEQ    mainLoop         ; If not active, loop back
            
            ; Check wave generation count
            LDD    pointCount       
            CPD    #POINTS_MAX      
            LBLO    mainLoop         
            
            ; Wave generation complete - clean up and accept new input
            CLR    waveActive       
            BCLR   TIE,%00100000    
...
;***********************************************************************
; Timer OC5 interrupt service routine - Handles ADC and wave generation
;***********************************************************************
oc5isr
            ; Schedule next interrupt to maintain precise 125us timing
            LDD    #3000            ; 125us interval at 24MHz bus clock
            ...
            ; Check if ADC generation is active
            LDAA   adcActive
            BNE    handleADC        ; Branch to ADC handling
            
            ; Handle standardr wave generation (from HW10)
            LDD    pointCount
            CPD    #POINTS_MAX
            LBHS    oc5done          
...
; ADC Handling. It handles the generation of data and conversions
handleADC
            ; Process ADC data acquisition
            LDD    adcCount
            CPD    #POINTS_MAX+1    ; we need to collect 2049 total samples (2048 printed, skip the -1)
            LBHS    oc5done         ; Skip if we've reached max points
            
            ; Check if conversion is complete
            LDAA   ATDSTAT0
            ANDA   #%10000000      ; Check SCF bit
            LBEQ    oc5done         ; If not complete, skip this cycle
            
            ; Get ADC result value
            LDAA   ATDDR0L         ; Read low byte of result register 0
            STAA   adcValue        ; Store for processing
            
            ; Start a new ADC conversion on channel 7
            LDAA   #%10000111      ; Single conversion, channel 7
            STAA   ATDCTL5
            
            ; Skip printing the first sample (when adcCount = 0)
            LDD    adcCount
            CPD    #0
            BEQ    skipPrintingFirstSample
            
            ; Output the ADC value directly as decimal value
            LDAB   adcValue
            CLRA
            JSR    printDecimal    ; Use special printing routine for ADC values
            
skipPrintingFirstSample
            ; Increment ADC count after successful conversion 
            ; (it was loading -1 - to 2046, rather than 0-2047)
            ; Personal Note: Please note to add D+1 if asked on exam
            LDD    adcCount
            ADDD   #1
            STD    adcCount
...
genSquare
            ; Generate square wave (0 for 0V, 255 for 5V)
            LDAB   waveValue        ; Print current value
            CLRA
            JSR    pnum10
...
updateTime
            ...
            LDX    ctr2p5m
            CPX    #162            ; Using 162 (84x2)cycles because TA's clock was 0.5ticks a second on HW 9
            ; This should creat a tick per second.
...
displayHours
            ; Convert ASCII hours for 7-segment decimal display
            LDAA   hourTens
...
displayMinutes
            ; Convert ASCII minutes for 7-segment decimal display
            LDAA   minTens
            SUBA   #'0'            
            LSLA                   
            LSLA
...
displaySeconds
            ; Convert ASCII seconds for 7-segment decimal display
            LDAA   secTens
            SUBA   #'0'            
            LSLA                   
            LSLA
...
handleInput CMPA   #CR             ; Check for Enter key
            ...
            LDAB   cmdCount
            CMPB   #19             ; Max buffer size - 1 (2047 not 2048, starting from 0)
            LBHS    inputDone       
...
processCommand
            ...
            CMPA   #'a'            ; ADC  acquisition command?
            LBEQ    adcAcquireCmd
...
;*************************
; ADC Acquiring command
;*************************
adcAcquireCmd
            ; Check for exactly "adc" input from user
            LDAA   cmdCount
            CMPA   #3              ; 3 cmd?
            LBNE    invalidCmd
            
            LDAA   1,X             ; Check second character
            CMPA   #'d'
            LBNE    invalidCmd
            
            LDAA   2,X             ; Check third character 
            CMPA   #'c'
            LBNE    invalidCmd
            
            ; Prepare for ADC acquisition
            JSR    nextline
            LDX    #processIndent
            JSR    printmsg
            LDX    #adcStartMsg
            JSR    printmsg
            JSR    nextline        ; Add a newline before values start
            
            ; Set ADC acquisition status to active
            LDAA   #1
            STAA   adcActive
            ...
            ; Set up ADC for first conversion
            LDAA   #%10000111      ; Single conversion, channel 7
            STAA   ATDCTL5
            
adcInitWait LDAA   ATDSTAT0        ; Wait for conversion to complete
            ANDA   #%10000000      ; Check SCF bit
            BEQ    adcInitWait     ; If not set, keep waiting
            
            ; Discard stale data (I had an issue where the first data read was not from the waveform)
            LDAA   ATDDR0L         ; Read first result but don't use it
            
            ; Start fresh acquiring of data from the 1st entry (conversion begins here)
            LDAA   #%10000111      ; Start another conversion
            STAA   ATDCTL5
            
            ; Configure timer for first interrupt (125us from now)
            LDD    #3000           ; 3000 timer ticks = 125us at 24MHz
            ...
            ; skipping cmd cleanup to avoid mischheduling data 
            ; directly returns without showing the prompt
            PULY                   ; Restore registers and return
            PULX
            PULB
            PULA
            RTS                    ; Return without going through cmdCleanup
...
quitCmd     
            ...
            JSR    nextline
            LDX    #processIndent  ; Add indentation for readability
            JSR    printmsg
...
;***********************************************************************
; Print decimal directly - Special version for ADC output
; Prints just the number followed by a newline without other formatting
;***********************************************************************
printDecimal
            PSHD                  
            PSHX
...
printDecimal1
            LDX    #10             ; Divide by 10
            IDIV                   ; D / X -> X=quotient, D=remainder
            LBEQ    printDecimal2   ; If quotient is 0, done dividing
            STAB   1,Y+            ; Store remainder
            INC    CTR             ; Count digit
            TFR    X,D             ; Quotient becomes new dividend
            LBRA    printDecimal1   
...
printDecimal2
            STAB   1,Y+            ; Store final digit
            INC    CTR             
            
printDecimal3
            LDAA   #$30            ; Convert to ASCII
            ADDA   1,-Y            
            JSR    putchar         ; Print digit
            DEC    CTR             
            LBNE    printDecimal3   
...
welcomeMsg          DC.B  "Welcome to Abdullah's Analog Signal + Wave Generator with Digital Clock program", $00
instructMsg3        DC.B  '1. Use "adc" command to acquire 2048 samples from analog input to be coverted to a digital signal', $00
instructMsg4        DC.B  '2. Use "gw", "gw2", "gt", "gq", "gq2" commands to generate some standard waves', $00
instructMsg5        DC.B  '3. Use "t HH:MM:SS", "h", "m", "s" commands to control the digital clock', $00

; Command and prompt messages
promptMsg           DC.B  'HW11> ', $00                ; > Indicator prompt
errorPrompt         DC.B  'Error> ', $00               ; Error message prompt
invalidMsg          DC.B  'Invalid input format', $00
invalidTimeMsg      DC.B  'Invalid time format. Correct clock => 00:00:00 to 23:59:59', $00
invalidCmdMsg       DC.B  'Invalid command. ("s" for second, "m" for minute, "h" for hour displays and "q" for "quit")', $00
processIndent       DC.B  '       ', $00               ; 7 spaces for indentation

; ADC operation messages
adcStartMsg         DC.B  'Analog signal beginning ....', $00
adcCompleteMsg      DC.B  'Analog signal completed.', $00
sampleCountMsg      DC.B  '2048 entries sampled!', $00

; Wave type messages
sawtoothStartMsg    DC.B  'sawtooth wave generation ....', $00
sawtooth125StartMsg DC.B  'sawtooth wave 100Hz generation ....', $00
triangleStartMsg    DC.B  'triangle wave generation ....', $00
squareStartMsg      DC.B  'square wave generation ....', $00
square125StartMsg   DC.B  'square wave 100Hz generation ....', $00
waveCompleteMsg     DC.B  'Wave generation complete. 2048 points generated.', $00

; Quit and typewriter mode messages
quitMsg             DC.B  'Wave program stopped. and', $00
typewriterMsg       DC.B  'typewriter mode enabled.', $00
